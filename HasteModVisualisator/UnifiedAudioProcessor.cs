...
public class UnifiedAudioProcessor : MonoBehaviour
{
    ...
    private void ApplyGaussianSmoothing()
    {
        ...
        int kernelSize = VisualizerConfig.GaussianKernelSize.Value;
        float intensity = VisualizerConfig.GaussianKernelIntensity.Value;
    ...
    void Update()
    {
        ...
        if (VisualizerConfig.QualityLevel.Value == 1 && Time.frameCount % 2 == 0) return;
        if (VisualizerConfig.QualityLevel.Value == 0 && Time.frameCount % 3 == 0) return;
    ...
    void ConfigureBeatDetectors()
    {
        ...
        float[] mins = VisualizerConfig.ParseCsvFloats(VisualizerConfig.Band_MinThresholds?.Value ?? "", expected, defaultMinThresholds);
        float[] decays = VisualizerConfig.ParseCsvFloats(VisualizerConfig.Band_DecayRates?.Value ?? "", expected, defaultDecayRates);
        float[] sens = VisualizerConfig.ParseCsvFloats(VisualizerConfig.Band_Sensitivities?.Value ?? "", expected, defaultSensitivities);
        float[] thrMult = VisualizerConfig.ParseCsvFloats(VisualizerConfig.Band_ThresholdMultipliers?.Value ?? "", expected, defaultThresholdMultipliers);
        float[] cooldowns = VisualizerConfig.ParseCsvFloats(VisualizerConfig.Band_Cooldowns?.Value ?? "", expected, defaultCooldowns);
        float[] MinExceed = VisualizerConfig.ParseCsvFloats(VisualizerConfig.Band_MinExceed?.Value ?? "", expected, defaultMinExceed);
        float[] beatDecays = VisualizerConfig.ParseCsvFloats(VisualizerConfig.Band_BeatDecays?.Value ?? "", expected, defaultBeatDecays);
    ...
    public class BeatDetector
    {
        ...
        public void Reset()
        {
            ...
            float historySeconds = VisualizerConfig.HistoryDuration.Value * 0.25f;
        ...
        public void Detect(float energy, float bandIndex, bool isSilent = false)
        {
            ...
            float sensitivityAdj = Mathf.Max(adaptiveThreshold.sensitivity, 0.001f) * VisualizerConfig.BeatSensitivity.Value;
            ...
            effectiveThreshold = currentThreshold * VisualizerConfig.BeatThreshold.Value * Mathf.Max(thresholdSensitivity, 0.001f);
            ...
            if (!_inResetPhase)
            {
                ...
                if (thresholdExcess > MinExceed)
                {
                    ...
                    activationSnapshot = VisualizerConfig.ShowDebug.Value
            ...
            if (beatDetected && VisualizerConfig.ShowDebug.Value && !Snapped)
            {
