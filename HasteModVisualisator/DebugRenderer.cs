...
public class DebugRenderer : MonoBehaviour
{
    ...
    void OnGUI()
    {
        if (!VisualizerConfig.ShowDebug.Value || audioProcessor == null) return;
    ...
    {
        if (!VisualizerConfig.ShowDebug.Value || audioProcessor == null)
        {
            audioDebugText = "";
            skyboxDebugText = "";
            csvDebugText = "";
        ...
        audioSB.AppendLine($"Quality: {VisualizerConfig.QualityLevel.Value}");
        audioSB.AppendLine($"Intensity: {VisualizerConfig.Intensity.Value:F2}");
        ...
        if (VisualizerConfig.ShowCSVDebug.Value == true)
        {
